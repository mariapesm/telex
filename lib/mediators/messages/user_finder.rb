module Mediators::Messages
  ...
  class AppUserFinder < UserFinder
    ...
    def get_users_from_heroku
      ...
      if owner_email.ends_with?('@herokumanager.com')
      ...
      collabs = collab_response.map do |row|
        next if owners.detect { |owner| owner[:email] == row.fetch('user').fetch('email') }
        extract_user(:collaborator, row.fetch('user'))
      end
      collabs.compact!

      self.users_details = owners + collabs
    end
  end
end
